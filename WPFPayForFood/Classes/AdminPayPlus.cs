...
using WPFPayForFood.Classes.DB;
using WPFPayForFood.Classes.Printer;
using WPFPayForFood.Classes.Scanner;
...
namespace WPFPayForFood.Classes
{
    public class AdminPayPlus : INotifyPropertyChanged
    {
        ...
        public Action<bool> callbackResult;
        ...
        private async Task<bool> LoginPaypad()
        {
            ...
            {
                Error.SaveLogError(MethodBase.GetCurrentMethod().Name, "AdminPayPlus", ex, ex.ToString());
            }
            return false;
        }
        ...
        private void ValidatePeripherals()
        {
            try
            {
                ...
                _controlPeripherals.callbackError = error =>
                {
                    ...
                    {
                        Reference = "",
                        Description = string.Concat(error.Item1, " ", error.Item2),
                        State = 2,
                        Date = DateTime.Now
                    }, ELogType.General);
        ...
        private CONFIGURATION_PAYDAD LoadInformation()
        {
            try
            {
                ...
                {
                    Error.SaveLogError("LoadInformation", "AdminPayPlus", null, "No se encontro el archivo de Keys.");
                }
            }
            catch (Exception ex)
            {
        ...
        public static async Task<int> SendPhone(SMSModel sms)
        {
            try
            {
                var response = await api.CallApi("SendMessagePhone", sms);
            }
            catch (Exception ex)
            {
