...
using WPFPayForFood.Classes.Peripherals;
...
namespace WPFPayForFood.Classes
{
    public class ControlPeripherals
    {
        ...
        public Action<string> callbackError_;//Calback de error
        ...
        public ControlPeripherals(string portNameBills, string denominatios, int dividerBills = 1, int dividerCoins = 100)
        {
            ...
            {
                callbackError?.Invoke(Tuple.Create("", string.Concat("Error (ControlPeripherals) EX: ", ex)));
            }
        }
        ...
        public void Start()
        {
            ...
            {
                callbackError?.Invoke(Tuple.Create("", string.Concat("Error (Start) EX: ", ex)));
            }
        }
        ...
        private void _serialPortBillsDataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                {
                    Reference = "",
                    Description = "Respuesta de los dispositivos: " + response,
                    State = 1,
                    Date = DateTime.Now
                }, ELogType.General);
